...
### Преподаватель
- Дашборд: три карточки статистики — «На проверке», «Просрочено», «Предметов»; клик по карточке применяет фильтр
- Карточки работ с цветовой полоской по статусу, фильтр по предмету и статусу (без перезагрузки страницы)
...
- История зачтённых работ: тип сдачи (электронная / в тетради), кнопка скачивания только для электронных
- Смена пароля в профиле
...
- Дашборд: студентов / преподавателей / на проверке / просрочено; карточки «На проверке» и «Просрочено» кликабельны — показывают детали по каждой работе (студент, задание, преподаватель)
- Журнал действий (вкладка «Бэклог»): полная история всех изменений с временными метками
- Мобильная навигация: dropdown «Раздел ▾» вместо горизонтального скролла вкладок
- Сброс пароля любого пользователя
- Экспорт успеваемости в CSV (имя файла и статусы — на русском языке)
- Система объявлений: публикация баннера для всех студентов, личные сообщения конкретному студенту
...
announcements         — объявления для студентов (баннер)
personal_messages     — личные сообщения студентам от администратора / преподавателя
